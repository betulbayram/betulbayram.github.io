document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (!blogContainer || !filterContainer || !paginationContainer) {
    console.error("Gerekli HTML elementleri (blog, filtre veya sayfalama taşıyıcısı) bulunamadı.");
    return;
  }
...
function render() {
    ...
    if (postsToShow.length === 0) {
        blogCardsHTML = "<p style='color: #adadad; text-align: center; width: 100%;'>Bu filtreyle eşleşen blog yazısı bulunamadı.</p>";
    } else {
    ...
    if (totalPages > 1) {
      if (currentPage > 1) {
        paginationHTML += `<a href="#" class="arrow" data-page="${currentPage - 1}">&laquo; Önceki</a>`;
      }
      ...
      if (currentPage < totalPages) {
        paginationHTML += `<a href="#" class="arrow" data-page="${currentPage + 1}">Sonraki &raquo;</a>`;
      }
    }
